/*
 * $Id: call_pipeline.c,v 1.20 2001/08/28 17:29:23 gerald Exp $
 */
...
void call_pipeline (trace_t *tr, FILE *fin, 
  ...
  int sock_length = sizeof(myaddr);
  
  if(!getpeername(sock_fd, (struct sockaddr *)&myaddr, &sock_length)) {
    trace(NORM, default_trace, "irr_submit connection from %s\n",
  ...
  if (ci.log_dir != NULL) {
    ...
    if ((ack_lock = lock_file (&lock_fd, logfn)) != 0)
      trace (ERROR, default_trace, "Error opening %s: %s\n",
    ...
    if ((trans_fp = fopen (logfn, "a")) != NULL) {
      ...
      trace (ERROR, default_trace, "Could not open transaction log: %s\n", 
	     logfn);
  }
  else
    trace (ERROR, default_trace, 
	   "No pipeline submission log directory configured!\n");
  ...
  if (ret_code > 0) {
    ...
    if (auth_check (tr, outfn, outfn2, ci.irrd_host, 
      ...
      trace (NORM, tr, "** FAILED AUTHCHECK **\n");
    }
    unlink (outfn);
  }
  ...
  dfile = stderr;
  callnotify (tr, outfn2, ret_code, null_notifications, dump_stdout,
