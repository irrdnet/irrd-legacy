...
int pgpdecodefile_new (FILE *infile, char *outfn, FILE *log_fd, 
  ...
  my_mktemp (tr, outfn);
  umask (0000);
  if ((pipeline_file = fopen (outfn, "w+")) == NULL) {
    fprintf (stderr, "Pipeline open file error %s: %s.\n", 
  ...
  while (fgets (curline, MAXLINE + 8, infile) != NULL) {
	 ...
	 * input by pgpv */
        ...
        my_mktemp (tr, pgpinfn);

	/* open the file for input to pgpv */
	if ((pgpinfile = fopen (pgpinfn, "w")) == NULL) {
	  trace (ERROR, tr, "pgpdecodefile () fopen error (%s)! (%s)\n", 
		 pgpinfn, strerror (errno));
	  inpgpsig = 0;
	  fputs (curline, pipeline_file);
	}
	else {
